...
export function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-4 pb-[calc(6rem+env(safe-area-inset-bottom))] md:space-y-6 md:pb-0">
      <SurfaceHeader
        ...
        aside={
          <div className="rounded-[1.35rem] border border-ops-border-soft bg-ops-panel/55 p-4 md:rounded-[1.7rem] md:p-5">
            ...
            <div className="mt-3 space-y-2.5 text-sm leading-5 text-ops-muted md:mt-4 md:space-y-3 md:leading-6">
            ...
            <Link
              ...
              className="mt-3 inline-flex h-10 items-center justify-center rounded-2xl border border-ops-border bg-[linear-gradient(180deg,rgba(21,26,34,0.96),rgba(12,15,21,0.96))] px-4 text-sm font-semibold text-ops-text transition hover:border-ops-primary/35 hover:bg-ops-panel md:mt-4 md:h-11"
            >
      ...
      <div className="grid gap-4 xl:grid-cols-[0.88fr_1.12fr] md:gap-6">
        <div className="space-y-4 md:space-y-6">
          ...
          <PanelSection title="Priority board" description="Keep the next admin actions visible across review, dispatch, finance, and product intake.">
            <EntityList>
              <div className="rounded-[1.2rem] border border-ops-border-soft/90 bg-ops-surface/72 p-3.5 md:rounded-[1.4rem] md:p-4">
            ...
            <EntityList>
              {overdueDrivers.length ? (
                overdueDrivers.map((driver) => (
                  <div key={driver.driverId} className="rounded-[1.2rem] border border-ops-border-soft/90 bg-ops-surface/72 p-3.5 md:rounded-[1.4rem] md:p-4">
            ...
            <EntityList>
              {moderationQueue.length ? (
                moderationQueue.map(({ proposal, meta }) => (
                  <div key={proposal.id} className="rounded-[1.2rem] border border-ops-border-soft/90 bg-ops-surface/72 p-3.5 md:rounded-[1.4rem] md:p-4">
                    <div className="flex flex-wrap items-center gap-2">
          ...
          <EntityList>
            {rides.length ? (
              rides.map((ride) => {
                ...
                return (
                  <div key={ride.id} className="rounded-[1.3rem] border border-ops-border-soft/90 bg-ops-surface/72 p-4 md:rounded-[1.6rem] md:p-5">
                    <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                      <div className="min-w-0">
                        ...
                        <div className="mt-3 grid gap-2.5 lg:grid-cols-2 md:gap-3">
          ...
          <EntityList>
            {riderLeads.length ? (
              riderLeads.slice(0, 6).map((lead) => (
                <div key={lead.id} className="rounded-[1.2rem] border border-ops-border-soft/90 bg-ops-surface/72 p-3.5 md:rounded-[1.4rem] md:p-4">
          ...
          <EntityList>
            {driverInterests.length ? (
              driverInterests.slice(0, 6).map((interest) => (
                <div key={interest.id} className="rounded-[1.2rem] border border-ops-border-soft/90 bg-ops-surface/72 p-3.5 md:rounded-[1.4rem] md:p-4">
