...
export function DriverActiveRideCard({ ride, emphasize, mobileDocked = false }: DriverActiveRideCardProps) {
  ...
  if (mobileDocked) {
    return (
      <div className={`rounded-[0.95rem] border px-3 py-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)] ${emphasize ? "border-teal-500/20 bg-teal-500/[0.05]" : "border-white/10 bg-slate-950/24"}`}>
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-400">Current trip</p>
            ...
            <p className="truncate text-[11px] text-slate-400">{formatDriverMilesCompact(ride.estimatedMiles)} · {formatDriverMinutesCompact(ride.estimatedMinutes)}</p>
          </div>
          <div className="flex shrink-0 items-center gap-3">
            ...
            <Link
              ...
              className="inline-flex h-8 items-center justify-center rounded-lg border border-teal-500/22 bg-teal-500/12 px-3 text-sm font-semibold text-white transition hover:bg-teal-500/18"
            >
              Resume
